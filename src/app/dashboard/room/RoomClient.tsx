...
type InitialData = Awaited<ReturnType<typeof getRoomData>>
type BulkData = Awaited<ReturnType<typeof getBulkRoomData>>

export function RoomClient({ initialData, initialBulkData }: { initialData: InitialData; initialBulkData: BulkData }) {
    // ⚡ Kiểm tra quyền bulk từ server data
    const bulkOk = !('error' in initialBulkData && initialBulkData.error)
    const initBulkRooms = bulkOk && 'rooms' in initialBulkData ? (initialBulkData as any).rooms || [] : []
    const initBulkReports = bulkOk && 'reports' in initialBulkData ? (initialBulkData as any).reports || [] : []

    const [room, setRoom] = useState<{ name: string; default_capacity: number } | null>(
        (initialData.room as { name: string; default_capacity: number }) || null
    )
    const [isWithinTime, setIsWithinTime] = useState(() => {
        if (bulkOk && 'isWithinTime' in initialBulkData) return initialBulkData.isWithinTime as boolean
        return initialData.isWithinTime ?? true
    })
    const [reportDate, setReportDate] = useState(() => {
        if (bulkOk && 'reportDate' in initialBulkData) return (initialBulkData.reportDate as string) ?? ''
        return initialData.reportDate ?? ''
    })
    const [phaseLabel, setPhaseLabel] = useState(() => {
        if (bulkOk && 'phaseLabel' in initialBulkData) return (initialBulkData.phaseLabel as string) ?? ''
        return initialData.phaseLabel ?? ''
    })
    const [existingReport, setExistingReport] = useState<Record<string, unknown> | null>(
        (initialData.report as Record<string, unknown>) || null
    )
    const [loading, setLoading] = useState(false)
    const [submitting, setSubmitting] = useState(false)
    const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(
        'error' in initialData && initialData.error && !bulkOk ? { type: 'error', text: initialData.error as string } : null
    )
    
    // ⚡ Khởi tạo viewMode từ bulk data — admin không có phòng riêng → mặc định bulk
    const [viewMode, setViewMode] = useState<'single' | 'bulk' | 'teacher'>(() => {
        if (bulkOk && !initialData.room) return 'bulk'
        return 'single'
    })
    
    // Admin / approver roles — khởi tạo từ server data
    const [hasBulkAccess, setHasBulkAccess] = useState(bulkOk)
    const [bulkData, setBulkData] = useState<{rooms: any[], reports: any[]} | null>(
        bulkOk ? { rooms: initBulkRooms, reports: initBulkReports } : null
    )
    const [bulkPhase, setBulkPhase] = useState<string>(
        bulkOk && 'phase' in initialBulkData ? (initialBulkData as any).phase || 'moc1' : 'moc1'
    )

    // Form state
    const [capacity, setCapacity] = useState(() => {
        if (initialData.report) return (initialData.report as Record<string, unknown>).capacity as number;
        if (initialData.room) return (initialData.room as { default_capacity: number }).default_capacity || 0;
        return 0;
    })
    const [absentCount, setAbsentCount] = useState(() => 
        initialData.report ? (initialData.report as Record<string, unknown>).absent_count as number : 0
    )
    const [porridgeCount, setPorridgeCount] = useState(() => 
        initialData.report ? (initialData.report as Record<string, unknown>).porridge_count as number : 0
    )
    const [vegetarianCount, setVegetarianCount] = useState(() => 
        initialData.report ? (initialData.report as Record<string, unknown>).vegetarian_count as number : 0
    )
    const [note, setNote] = useState(() => 
        initialData.report ? ((initialData.report as Record<string, unknown>).note as string) || '' : ''
    )
    const [absentList, setAbsentList] = useState<AbsentStudent[]>(() => 
        initialData.report ? ((initialData.report as Record<string, unknown>).absent_list as AbsentStudent[]) || [] : []
    )
