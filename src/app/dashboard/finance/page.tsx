...
import {
    ...
    getDebtSummary,
    getTeacherDebtReport,
    TeacherDebtDay,
    getCompanyAndPaymentSettings,
    CompanyPaymentSettings,
    getPaymentRequests,
    PaymentRequest,
} from './actions'
import { VoucherForm, VoucherCreatedData } from './VoucherForm'
...
export default function FinancePage() {
    ...
    const [companySettings, setCompanySettings] = useState<CompanyPaymentSettings | null>(null)
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const [paymentsResult, debtResult, teacherDebtResult, companyResult, requestsResult] = await Promise.all([
                ...
                getCompanyAndPaymentSettings(),
                getPaymentRequests(startDate, endDate),
            ])
        ...
        <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-2xl p-5 border border-amber-200/80 shadow-sm transition-all hover:shadow-md bg-gradient-to-br from-white via-white to-amber-50/40">
                    ...
                    <div className="text-[10px] text-amber-600/90 mt-1 italic font-medium">
                        Đã gửi Giấy ĐNTT cho nhà trường
                    </div>
                </div>

                {/* Card 3: Đã thực thu vào quỹ */}
                <div className="bg-white rounded-2xl p-5 border border-emerald-100 shadow-sm transition-all hover:shadow-md">
                    <div className="flex justify-between items-start mb-2">
                        <div className="text-gray-500 text-[10px] font-bold uppercase tracking-wider">Đã thực thu vào quỹ</div>
                    ...
                    <div className="text-[10px] text-gray-400 mt-1 italic font-medium">
                        Tiền thực tế đã vào tài khoản/két
                    </div>
                </div>

                {/* Card 4: Công nợ thực tế */}
