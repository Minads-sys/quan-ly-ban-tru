...
import { Calendar as CalendarIcon, Filter, TrendingUp, Utensils, Beaker, FileSpreadsheet, Printer, FileText } from 'lucide-react'
import { ReconciliationPrint } from '../finance/ReconciliationPrint'
...
export function ReportsClient({ initialData }: { initialData: Awaited<ReturnType<typeof getReportsData>> }) {
    const [filter, setFilter] = useState<ReportFilter>({ period: 'this_month' })
    const [customStart, setCustomStart] = useState('')
    const [customEnd, setCustomEnd] = useState('')
    
    const [loading, setLoading] = useState(false)
    const [errorMsg, setErrorMsg] = useState('error' in initialData ? initialData.error as string : '')
    const [summary, setSummary] = useState<ReportSummary | null>('error' in initialData ? null : initialData as ReportSummary)
    const [chartType, setChartType] = useState<'line' | 'bar'>('line')
    const [showReconciliation, setShowReconciliation] = useState(false)
    const [reconciliationType, setReconciliationType] = useState<'student' | 'teacher'>('student')
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between flex-wrap gap-3">
                            ...
                            <div className="flex items-center gap-2 print:hidden">
                                <button
                                    id="btn-reconciliation"
                                    onClick={() => {
                                        setReconciliationType('student')
                                        setShowReconciliation(true)
                                    }}
                                    className="flex items-center gap-1.5 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-xl shadow-sm transition-all cursor-pointer active:scale-95"
                                    title="Tạo Biên bản đối chiếu cung cấp suất ăn chuẩn Khổ A4 Dọc"
                                >
                                    <FileText className="w-4 h-4" />
                                    Biên bản đối chiếu (A4 Dọc)
                                </button>
                                <button
                                    id="btn-export-excel"
