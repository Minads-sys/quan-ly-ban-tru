...
export function KitchenClient({ initialData }: { initialData: InitialData }) {
    ...
    const [activeTab, setActiveTab] = useState<'kitchen' | 'distributor' | 'teacher'>(initRole === 'meal_distributor' ? 'distributor' : 'kitchen')
    const [totalSalty, setTotalSalty] = useState(initialData.totalSalty as number || 0)
    const [totalVegetarian, setTotalVegetarian] = useState(initialData.totalVegetarian as number || 0)
    const [totalPorridge, setTotalPorridge] = useState(initialData.totalPorridge as number || 0)
    const [totalMeals, setTotalMeals] = useState(initialData.totalMeals as number || 0)
    const [totalCong, setTotalCong] = useState(initialData.totalCong as number || 0)
    const [groupSummaries, setGroupSummaries] = useState<GroupSummary[]>((initialData.groupSummaries as GroupSummary[]) || [])
    const [userRole, setUserRole] = useState(initRole)
    const [isAfter14h, setIsAfter14h] = useState(false)
    const [showSummaryOnly, setShowSummaryOnly] = useState(initRole === 'kitchen' || initRole === 'meal_distributor')
    const [schoolInfo, setSchoolInfo] = useState(initialData.schoolInfo || { name: '', address: '' })
    const [moc1Close, setMoc1Close] = useState((initialData.moc1Close as string) || '16:00')
    const [isLocked, setIsLocked] = useState(false)
    const [isMoc1Closed, setIsMoc1Closed] = useState(!!initialData.isMoc1Closed)
    const [isMoc2Closed, setIsMoc2Closed] = useState(!!initialData.isMoc2Closed)
    const [workingDays, setWorkingDays] = useState<number[]>((initialData.workingDays as number[]) || [1, 2, 3, 4, 5])
    const [offDays, setOffDays] = useState<string[]>((initialData.offDays as string[]) || [])
    const [dayOffMessage, setDayOffMessage] = useState<string | null>(null)

    // Teacher meal states — khởi tạo từ initialData
    const [teacherSalty, setTeacherSalty] = useState(initTeacher?.salty_count || 0)
    const [teacherPorridge, setTeacherPorridge] = useState(initTeacher?.porridge_count || 0)
    const [teacherVegetarian, setTeacherVegetarian] = useState(initTeacher?.vegetarian_count || 0)
    const [teacherNote, setTeacherNote] = useState(initTeacher?.note || '')
    const [teacherTotal, setTeacherTotal] = useState((initTeacher?.salty_count || 0) + (initTeacher?.porridge_count || 0) + (initTeacher?.vegetarian_count || 0))
    const [teacherReportExists, setTeacherReportExists] = useState(!!initTeacher)
    ...
    const [teacherFormSalty, setTeacherFormSalty] = useState(initTeacher?.salty_count || 0)
    const [teacherFormPorridge, setTeacherFormPorridge] = useState(initTeacher?.porridge_count || 0)
    const [teacherFormVegetarian, setTeacherFormVegetarian] = useState(initTeacher?.vegetarian_count || 0)
    const [teacherFormNote, setTeacherFormNote] = useState(initTeacher?.note || '')

    const dateRef = useRef((initialData.date as string) || '')
    ...
    // ⚡ Bỏ useEffect mount — data đã có sẵn từ server (initialData)
    // loadData() chỉ dùng cho Realtime refresh và date-change

    // ⚡ Realtime: tự động làm mới khi có thay đổi daily_reports hoặc teacher_meal_reports
    // Chỉ lắng nghe event của ngày đang xem (filter theo report_date)
    useRealtimeRefresh(
        ['daily_reports', 'teacher_meal_reports'],
        loadData,
        1500,
        date ? { column: 'report_date', value: date } : undefined
    )
